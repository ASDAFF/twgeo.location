...
if ($request->get('request') == 'getcities')
{
    try{
        ...
        switch (Settings::get('TWG_LOCATION_LOAD_LOCATIONS')):
            case 'cities':
...
if ($request->get('request') == 'setcity')
{
    ...
    $reload     = Settings::get('TWG_LOCATION_RELOAD') == 'Y';
    $redirect   = false;
    ...
    if ($locationName)
    {
        ...
        //
        if (Settings::get('TWG_LOCATION_REDIRECT') != 'N') {
            $domain = \TWGeo\Location\Iblock\Content::getDomainByLocationId($locationId);
            if (strlen($domain)) {
