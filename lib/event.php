...
class Event
{
    ...
    public static function onBeforeProlog()
    {
        ...
        $event = new \Bitrix\Main\Event("twgeo.location", "afterLocationDetect");
        $event->send();

        // redirect if needed
        if (!Storage::isEmpty() && (Settings::get('TWG_LOCATION_REDIRECT') == 'C'))
        {
    ...
    public static function setSaleLocation( &$arUserResult, Request $request, &$arParams, &$arResult)
    {
        ...
        if ($settings['TWG_LOCATION_DELIVERY'] != 'Y')
            return;
        ...
        if ($settings['TWG_LOCATION_TEMPLATE'] == 'Y')
            $arParams['TEMPLATE_LOCATION'] = 'twg_location';
        ...
        if (!Storage::isEmpty())
        {
            ...
            if (($settings['TWG_LOCATION_DELIVERY_ZIP'] == 'Y') && $zipPropertyId) {
 ...
 */
    ...
    public static function setZip($order, &$arUserResult, $request, &$arParams, &$arResult)
    {
        ...
        if ($settings['TWG_LOCATION_DELIVERY_ZIP'] != 'Y') return;
