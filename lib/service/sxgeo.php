...
namespace TWGeo\Location\Service;

use Bitrix\Main\Config\Option;
use TWGeo\Location\Service;
 ...
 *
 */
class SxGeo extends Service
{
    protected static $url = 'https://sypexgeo.net/files/SxGeoCountry.zip';
    ...
    public function get($ip){
        return $this->max_city ? $this->getCityFull($ip) : $this->getCountry($ip);
    }
    ...
    {
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/twgeo.location/include/SxGeoCity.dat'))
            return [];

        mb_internal_encoding("cp-1251");

        $SxGeo = new SxGeo(
            $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/twgeo.location/include/SxGeoCity.dat',
            Settings::get('TWG_LOCATION_SXGEO_MEMORY') ? SXGEO_MEMORY : SXGEO_FILE
        );
    ...
    {
        return $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/twgeo.location/include/';
    }
    ...
    public static function run()
    {
        ...
        chdir(self::getDir());
        $types = array('Country' => 'SxGeo.dat', 'City' => 'SxGeoCity.dat', 'Max' => 'SxGeoMax.dat',);
        // Скачиваем архив
        preg_match("/(Country|City|Max)/", pathinfo(self::$url, PATHINFO_BASENAME), $m);
        $type     = $m[1];
        $dat_file = $types[$type];

        $fp = fopen(self::getDir() . '/SxGeoTmp.zip', 'wb');
        $ch = curl_init(self::$url);
        curl_setopt_array(
            $ch, [
                CURLOPT_FILE => $fp,
                CURLOPT_HTTPHEADER => self::getLastUpdateGMT()
                    ? ["If-Modified-Since: " . self::getLastUpdateGMT()]
                    : []]
        );

        if (!curl_exec($ch))
            throw new SystemException(Loc::getMessage('TWGL__UPDATE_ERROR_DOWNLOAD'));

        $code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);
        fclose($fp);
        if ($code == 304) {
            @unlink(self::getDir() . '/SxGeoTmp.zip');

            return Loc::getMessage('TWGL__UPDATE_IDLE');
        }

        // Распаковываем архив
        $fp = fopen('zip://' . self::getDir() . '/SxGeoTmp.zip#' . $dat_file, 'rb');
        $fw = fopen($dat_file, 'wb');
        if (!$fp)
            throw new SystemException(Loc::getMessage('TWGL__UPDATE_ERROR_ARCHIVE_OPEN'));

        stream_copy_to_stream($fp, $fw);
        fclose($fp);
        fclose($fw);
        if (filesize($dat_file) == 0)
            throw new SystemException(Loc::getMessage('TWGL__UPDATE_ERROR_ARCHIVE_UNPACK'));

        @unlink(self::getDir() . '/SxGeoTmp.zip');
        if (!rename(self::getDir() . '/' . $dat_file, self::getDir() . $dat_file))
            throw new SystemException(Loc::getMessage('TWGL__UPDATE_ERROR_RENAME'));

        self::setLastUpdateGmt();

        return Loc::getMessage('TWGL__UPDATE_SUCCESS');
    }
     ...
     * 
     */
    public static function getLastUpdateGMT()
    {
        return Option::get('twgeo.location', 'sxgeo_last_update');
    }
     ...
     * 
     */
    public static function setLastUpdateGmt($gmt = null)
    {
        if (is_null($gmt))
            $gmt = gmdate('D, d M Y H:i:s');

        Option::set('twgeo.location', 'sxgeo_last_update', $gmt . ' GMT');
    }
}