...
use TwoFingers\Location\Service;
...
class SxGeo extends Service
{
    ...
    {
        if (!file_exists($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/twofingers.location/include/SxGeoCity.dat'))
            return [];

        mb_internal_encoding("cp-1251");

        $SxGeo = new SxGeo(
            $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/twofingers.location/include/SxGeoCity.dat',
            Settings::get('TF_LOCATION_SXGEO_MEMORY') ? SXGEO_MEMORY : SXGEO_FILE
        );
    ...
    {
        return $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/twofingers.location/include/';
    }
    ...
    public static function run()
    {
        ...
        if (!curl_exec($ch))
            throw new SystemException(Loc::getMessage('TFL__UPDATE_ERROR_DOWNLOAD'));
        ...
        if ($code == 304) {
            ...
            return Loc::getMessage('TFL__UPDATE_IDLE');
        }
        ...
        if (!$fp)
            throw new SystemException(Loc::getMessage('TFL__UPDATE_ERROR_ARCHIVE_OPEN'));

        stream_copy_to_stream($fp, $fw);
        fclose($fp);
        fclose($fw);
        if (filesize($dat_file) == 0)
            throw new SystemException(Loc::getMessage('TFL__UPDATE_ERROR_ARCHIVE_UNPACK'));

        @unlink(self::getDir() . '/SxGeoTmp.zip');
        if (!rename(self::getDir() . '/' . $dat_file, self::getDir() . $dat_file))
            throw new SystemException(Loc::getMessage('TFL__UPDATE_ERROR_RENAME'));

        self::setLastUpdateGmt();

        return Loc::getMessage('TFL__UPDATE_SUCCESS');
    }
    ...
    {
        return Option::get('twofingers.location', 'sxgeo_last_update');
    }
    ...
    public static function setLastUpdateGmt($gmt = null)
    {
        ...
        Option::set('twofingers.location', 'sxgeo_last_update', $gmt . ' GMT');
    }
}