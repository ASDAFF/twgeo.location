...
class Storage
{
    ...
    public static function getByKey($key, $driver = null)
    {
        ...
        if ($driver == self::TYPE__COOKIE)
            $data = $_COOKIE['TF_LOCATION'];
        else
            $data = $_SESSION['TF_LOCATION'];
    ...
    public static function setByKey($key, $value, $driver = null)
    {
        ...
        if ($driver == self::TYPE__COOKIE){
            $lifetime = intval(Settings::get('TF_LOCATION_COOKIE_LIFETIME'));
            setcookie('TF_LOCATION[' . $key . ']', $value, time() + 60 * 60 * 24 * $lifetime, '/');
            $_COOKIE['TF_LOCATION'][$key] = $value;
        } else {
    ...
    {
        $lifetime = Settings::get('TF_LOCATION_COOKIE_LIFETIME');
    ...
    public static function clear($driver = null)
    {
        ...
        if ($driver == self::TYPE__COOKIE){
            if (isset($_COOKIE['TF_LOCATION'])){
                foreach ($_COOKIE['TF_LOCATION'] as $key => $value){
                    unset($_COOKIE['TF_LOCATION'][$key]);
                    setcookie('TF_LOCATION[' . $key . ']', $value, time() - 60 * 60 * 24 * 7, '/');
                }
            }
        } else {
