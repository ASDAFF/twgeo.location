...
if(isset($_POST)) {
    if (isset($_POST['TF_LOCATION_SAVE_SETTINGS'])) {
        ...
        if($_POST['TF_LOCATION_TEMPLATE']=="Y"){
            CopyDirFiles($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/".$module_id."/install/location/", $_SERVER["DOCUMENT_ROOT"] . "/bitrix/templates/.default/components", true, true);
        }else{
...
if($MOD_RIGHT>="W" && true):
    ...
    <script>
        ...
        $().ready(function() {
            ...
            /* $(document).on('click', '.select_place', function(){
             ...
             $('.empty_location_el').remove();
             $('.tf_location_cities').append('<li data-id="'+locationID+'">'+LocationName+'<input type="hidden" value="'+locationID+'" name="TF_LOCATION_DEFAULT_CITIES[]"><i></i></li>');
             $('#LOCATION_tmp select').not('#COUNTRY_tmptmp').remove();
             $('#COUNTRY_tmptmp option').first().attr('selected', 'selected');
             }
             });*/
    ...
    <form method="post">
            ...
            <tr>
                ...
                <td width="60%" class="adm-detail-content-cell-r" valign="top">
                    <ul class="tf_location_cities">
                        <?if(empty($settings['TF_LOCATION_DEFAULT_CITIES'])):?>
                            <li class="empty_location_el"><?=GetMessage('TF_LOCATION_EMPTY_LIST');?></li>
                        <?else:?>
                            <?
                            ...
                            while ($vars = $db_vars->Fetch()):
                                ...
                                <li data-id="<?=$vars['ID']?>">
                                    <?=$vars['CITY_NAME']?>
                                    <input type="hidden" value="<?=$vars['ID']?>" name="TF_LOCATION_DEFAULT_CITIES[]"><i></i>
                                </li>
                            <?
                            endwhile;
                            ?>
        ...
        <?endif;?>
        <tr class="heading">
            <td colspan="2"><?=GetMessage('TF_LOCATION_JS_HEADER_BLOCK')?></td>
        </tr>
        <tr>
            <td width="40%" class="adm-detail-content-cell-l">
                <label for="TF_LOCATION_JQUERY_INCLUDE"><?=GetMessage('TF_LOCATION_JQUERY_INCLUDE')?>:</label>
            </td>
            <td width="60%" class="adm-detail-content-cell-r" valign="top">
                <input type="checkbox" name="TF_LOCATION_JQUERY_INCLUDE" id="TF_LOCATION_JQUERY_INCLUDE" value="Y" <?if ($settings['TF_LOCATION_JQUERY_INCLUDE'] == 'Y'):?> checked<?endif?>>
            </td>
        </tr>
        <?	$tab->EndTab();?>
        <?	$tab->Buttons();?>
